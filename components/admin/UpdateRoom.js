...
import { updateRoom, getRoomDetails } from '../../store/actions/roomAction'
import { UPDATE_ROOM_RESET } from '../../store/actioTypes/roomTypes'
...
const UpdateRoom = () => {
    ...
    const { loading, isUpdated, error } = useSelector(state => state.updateRoom)
    const { loading: roomDtailsLoading, room, error: roomDetailsError } = useSelector(state => state.roomDetails)
    ...
    }, [dispatch, error, isUpdated, id, room, roomDetailsError])
